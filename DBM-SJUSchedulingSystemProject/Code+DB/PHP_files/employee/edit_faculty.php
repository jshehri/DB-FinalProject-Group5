  ...
  if(isset($_POST['fname']))
	{
		$fname = mysql_real_escape_string($_REQUEST['fname']);
			...
			if ($rowCount === 0)
			{
				echo "<p class='errors'>There is no faculty member with this name! please try again</p>";
				echo "<br></br>";
			}
			else
			{
...
?>
		...
		<?php
			while ($row = mysqli_fetch_array($result)) {
				echo '<tr>';
					...
					echo '<td><a href="../employee/edit_faculty2.php?edit=' . $row['FID'] . '">Edit</a></td>';
					echo '<td><a href="../employee/edit_faculty.php?deleteID=' . $row['FID'] . '">Delete</a></td>';
				echo '</tr>';
			}}
			echo'</table>';
	}
		...
		<?php
			...
			{
				$FID = $_REQUEST['deleteID'];
				$deleteQuery = "DELETE FROM login WHERE UName='$FID'";

				if (mysqli_query($conn,$deleteQuery) === TRUE) 
				{
					echo "<p class='notice'>* Faculty member was deleted successfully.</p>";
				} 
				else 
				{
					echo "<p class='errors'>* System could not delete this faculty!</p>";
				}
			}
		?>
		<br>
